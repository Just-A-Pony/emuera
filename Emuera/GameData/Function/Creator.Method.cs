...
namespace MinorShift.Emuera.GameData.Function
{
	...
	internal static partial class FunctionMethodCreator
	{
			...
			public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
			{
				...
				try
				{
					...
					if (!System.IO.File.Exists(filepath))
						return 0;
					if (string.Equals(System.IO.Path.GetExtension(filepath), ".WEBP",StringComparison.OrdinalIgnoreCase))
					{
						using (WebP webp = new WebP())
							bmp = webp.Load(filepath);
						//throw new WebPLibException("WebP Library can not load", ex);
					}
					else
					{
				...
				catch (WebPLibException) { throw;  /*** <note> WebP Library Error may cause un-managed zone crash, so thrown. </note> ***/}
				catch (WebPException) { return 0; /*** <note> WebP Error including DLL not found return 0 </note> ***/}
				catch (Exception e)
				{
					if (e is CodeEE)
						throw;
		...
		/// </summary>
		/// <remarks>
		/// PNG only can. If you want to output images with webp or other format,
		/// it should to add argument for other type(s) and define that enum (BMP/TIFF/JPEG/webp/or so...)
		/// </remarks>
